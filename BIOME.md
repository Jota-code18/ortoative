...
| `DadosEstruturados.tsx`, `gravacao/page.tsx` | `noDangerouslySetInnerHtml` | JSON-LD exige `<script type="application/ld+json">` com conteúdo bruto. O dado é próprio, montado no servidor, sem nada vindo do usuário. A página de gravação injeta CSS fixo pelo mesmo motivo: esconder Header e rodapé para eles não entrarem no quadro. |
...
| `CenaClinicaGravacao.tsx`, `QuizInline.tsx` | `noExcessiveCognitiveComplexity` | Um monta uma cena three.js completa (é a cena dos passeios, preservada para regravar o vídeo); o outro é uma máquina de estados de triagem ramificada. Quebrar em funções menores espalharia estado sem reduzir a complexidade real. |
